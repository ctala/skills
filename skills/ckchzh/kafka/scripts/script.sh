...
DATA_DIR="${HOME}/.local/share/kafka"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "kafka v2.0.0"; }

_help() {
    echo "Kafka v2.0.0 — devtools toolkit"
    echo ""
    echo "Usage: kafka <command> [args]"
    echo ""
    echo "Commands:"
    echo "  check              Check"
...
case "${1:-help}" in
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: kafka check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [Kafka] check: $input"
    ...
    validate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/validate.log" 2>/dev/null || echo "  No entries yet. Use: kafka validate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/validate.log"
            local total=$(wc -l < "$DATA_DIR/validate.log")
            echo "  [Kafka] validate: $input"
    ...
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: kafka generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [Kafka] generate: $input"
    ...
    format)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/format.log" 2>/dev/null || echo "  No entries yet. Use: kafka format <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/format.log"
            local total=$(wc -l < "$DATA_DIR/format.log")
            echo "  [Kafka] format: $input"
    ...
    lint)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/lint.log" 2>/dev/null || echo "  No entries yet. Use: kafka lint <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/lint.log"
            local total=$(wc -l < "$DATA_DIR/lint.log")
            echo "  [Kafka] lint: $input"
    ...
    explain)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/explain.log" 2>/dev/null || echo "  No entries yet. Use: kafka explain <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/explain.log"
            local total=$(wc -l < "$DATA_DIR/explain.log")
            echo "  [Kafka] explain: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: kafka convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [Kafka] convert: $input"
    ...
    template)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/template.log" 2>/dev/null || echo "  No entries yet. Use: kafka template <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/template.log"
            local total=$(wc -l < "$DATA_DIR/template.log")
            echo "  [Kafka] template: $input"
    ...
    diff)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/diff.log" 2>/dev/null || echo "  No entries yet. Use: kafka diff <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/diff.log"
            local total=$(wc -l < "$DATA_DIR/diff.log")
            echo "  [Kafka] diff: $input"
    ...
    preview)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/preview.log" 2>/dev/null || echo "  No entries yet. Use: kafka preview <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/preview.log"
            local total=$(wc -l < "$DATA_DIR/preview.log")
            echo "  [Kafka] preview: $input"
    ...
    fix)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/fix.log" 2>/dev/null || echo "  No entries yet. Use: kafka fix <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/fix.log"
            local total=$(wc -l < "$DATA_DIR/fix.log")
            echo "  [Kafka] fix: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: kafka report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Kafka] report: $input"
