...
description 通过关键词搜索本地文档（Word、Excel、PDF、Markdown）。
...
```mermaid
flowchart TD
    A([开始]) -- B{已安装 cbridge？ncbridge --version}

    B -- 是 -- C[检测更新ncbridge update]
    C -- D{需要重新初始化？n询问用户}
    D -- 是 -- E[重新初始化配置ncbridge init]
    D -- 否 -- G
    E -- G

    B -- 否 -- F1[说明需安装 ContextBridgen用于监控本地目录、向量化文件]
    F1 -- F2[说明 ContextBridge 的工作目录 ~.cbridge ]
    F2 -- F3[获取用户授权n自动完成下载与初始化（默认配置）]
    F3 -- G

    G[配置完成n通过 cbridge 与 ContextBridge 交互] -- H([结束])
```
...
2. `cbridge seatch` 会返回与搜索相关的文档片段以及文档路径，根据文档片段判断是否需要根据文档路径获取文档全部内容
