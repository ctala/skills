#!/usr/bin/env bash
# volume — Volume reference tool. Use when working with volume in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
volume v$VERSION — Volume Reference Tool
...
## What is Volume?
Volume (volume) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with volume.

## Key Concepts
- Core volume principles and fundamentals
- How volume fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Volume Matters
Understanding volume is critical for:
...
## Getting Started
1. Understand the basic volume concepts
...
cmd_formulas() {
    cat << 'EOF'
# Volume — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Volume — Instruments & Tools Overview

## Primary Instruments
- Core tools used in volume operations
...
cmd_glossary() {
    cat << 'EOF'
# Volume — Key Terms & Definitions

## Core Terminology
- **Volume**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "volume v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: volume help"; exit 1 ;;
esac
