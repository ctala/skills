#!/usr/bin/env bash
# teacher-toolkit - Learning and study assistant
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TEACHER_TOOLKIT_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/teacher-toolkit}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
teacher-toolkit v$VERSION

Learning and study assistant

Usage: teacher-toolkit <command> [args]
...
case "${1:-help}" in
    ...
    test) shift; cmd_test "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "teacher-toolkit v$VERSION" ;;
