#!/usr/bin/env bash
# 90Daysofdevops — utility tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/90daysofdevops"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "90daysofdevops v2.0.0"; }

_help() {
    echo "90Daysofdevops v2.0.0 — utility toolkit"
    echo ""
    echo "Usage: 90daysofdevops <command> [args]"
    echo ""
    echo "Commands:"
    echo "  run                Run"
    echo "  check              Check"
...
_export() {
    ...
    case "$fmt" in
        ...
        csv)
            ...
            ;;
        txt)
            echo "=== 90Daysofdevops Export ===" > "$out"
            for f in "$DATA_DIR"/*.log; do
                [ -f "$f" ] || continue
...
_search() {
    local term="${1:?Usage: 90daysofdevops search <term>}"
...
case "${1:-help}" in
    run)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/run.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops run <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/run.log"
            local total=$(wc -l < "$DATA_DIR/run.log")
            echo "  [90Daysofdevops] run: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [90Daysofdevops] check: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [90Daysofdevops] convert: $input"
    ...
    analyze)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/analyze.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops analyze <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/analyze.log"
            local total=$(wc -l < "$DATA_DIR/analyze.log")
            echo "  [90Daysofdevops] analyze: $input"
    ...
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [90Daysofdevops] generate: $input"
    ...
    preview)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/preview.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops preview <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/preview.log"
            local total=$(wc -l < "$DATA_DIR/preview.log")
            echo "  [90Daysofdevops] preview: $input"
    ...
    batch)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/batch.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops batch <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/batch.log"
            local total=$(wc -l < "$DATA_DIR/batch.log")
            echo "  [90Daysofdevops] batch: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [90Daysofdevops] compare: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [90Daysofdevops] export: $input"
    ...
    config)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/config.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops config <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/config.log"
            local total=$(wc -l < "$DATA_DIR/config.log")
            echo "  [90Daysofdevops] config: $input"
    ...
    status)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/status.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops status <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/status.log"
            local total=$(wc -l < "$DATA_DIR/status.log")
            echo "  [90Daysofdevops] status: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: 90daysofdevops report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [90Daysofdevops] report: $input"
    ...
    *)
        echo "Unknown: $1 — run '90daysofdevops help'"
        exit 1
        ;;
esac