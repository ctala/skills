#!/usr/bin/env bash
# optimism — Optimism reference tool. Use when working with optimism in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
optimism v$VERSION — Optimism Reference Tool

Usage: optimism <command>
...
## What is Optimism?
Optimism (optimism) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with optimism.

## Key Concepts
- Core optimism principles and fundamentals
- How optimism fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Optimism Matters
Understanding optimism is critical for:
...
## Getting Started
1. Understand the basic optimism concepts
...
cmd_formulas() {
    cat << 'EOF'
# Optimism — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Optimism — Instruments & Tools Overview

## Primary Instruments
- Core tools used in optimism operations
...
cmd_glossary() {
    cat << 'EOF'
# Optimism — Key Terms & Definitions

## Core Terminology
- **Optimism**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "optimism v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: optimism help"; exit 1 ;;
esac
