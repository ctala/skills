#!/usr/bin/env bash
# uncle -- Analyze uncle operations. Use when you need to understand uncle mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${UNCLE_DIR:-$HOME/.uncle}"
...
show_help() {
    cat << EOF
uncle v$VERSION -- Analyze uncle operations. Use when you need to understand uncle mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: uncle <command> [args]
...
cmd_add() {
    local value="${1:?Usage: uncle add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: uncle search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: uncle remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "uncle v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: uncle help"; exit 1 ;;
esac
