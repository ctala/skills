...
def _save_image_from_result(result: OCRResult) -> OCRResult:
    """
    从 OCR 结果中提取并保存图片
    ...
    """
    if not FileSaver:
        logger.warning("FileSaver not available, skipping image save")
        return result

    # API 返回的数据结构: result.data['ImageInfo'][0]['ImageBase64']
    image_base64 = None
    ...
    if image_base64 and result.code == SUCCESS_CODE:
        try:
            saver = FileSaver()
            save_res = saver.save_image_from_base64(image_base64)
            if save_res["code"] == 0:
                # 只返回 path 字段，不包含其他 API 原始数据
                result.data = {"path": save_res["data"]["path"]}
                logger.info(f"Image saved to: {save_res['data']['path']}")
            else:
                result = OCRResult(
                    code=save_res["code"],
                    message=save_res["msg"],
                    data=save_res["data"]
                )
                logger.error(f"Failed to save image: {save_res['msg']}")
        except Exception as e:
            result = OCRResult(code=-1, message=f"File save failed: {str(e)}", data={})
            logger.error(f"Image save exception: {str(e)}", exc_info=True)
...
示例：
  ...
  python3 scan.py \\
    ...
    try:
        ...
        # 保存图片（如果返回结果包含图片）
        result = _save_image_from_result(result)
