...
feishu_sender = load_helper_module("audioclaw-skills-voice-reply", "feishu_audio_sender.py", "rehearsal_feishu_sender")
...
def slugify(value: str) -> str:
    ...
    return cleaned.strip("_") or "rehearsal"
...
def select_counterpart_turns(history: dict, turn_indexes: str, limit: int) -> list[dict]:
    selected = [turn for turn in history.get("turns", []) if turn.get("kind") == "counterpart" and turn.get("audio_path")]
    if turn_indexes.strip():
        wanted = {item.strip() for item in turn_indexes.split(",") if item.strip()}
        selected = [turn for turn in selected if str(turn.get("turn_index")) in wanted]
    if limit > 0:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Send rehearsal counterpart turns to Feishu one by one as audio messages.")
    ...
    parser.add_argument("--turn-indexes", default="")
    ...
    selected_turns = select_counterpart_turns(history, args.turn_indexes, args.limit)
    if not selected_turns:
        raise SystemExit("no counterpart turns selected for Feishu sending")
    ...
    if args.outdir.strip():
        ...
        prepared_dir = workspace_root / "state" / "audio" / "rehearsal"
    prepared_dir.mkdir(parents=True, exist_ok=True)

    deliveries = []
    for index, turn in enumerate(selected_turns, start=1):
        source_path = Path(str(turn.get("audio_path") or "")).expanduser().resolve()
        if not source_path.exists():
            raise SystemExit(f"missing synthesized rehearsal audio file: {source_path}")
        target_name = f"turn_{int(turn.get('turn_index', index)):02d}_{slugify(str(turn.get('phase') or 'counterpart'))}.ogg"
        target_path = prepared_dir / target_name
        transcode_to_ogg(source_path, target_path, ffmpeg_exe=ffmpeg_exe)

        if args.send_labels:
            phase = str(turn.get("phase") or "counterpart")
            send_text = send_text_message(chat_id, f"演练 {phase}", tenant_token)
        else:
            send_text = None

        duration_ms = (((turn.get("extra_info") or {}).get("audio_length")) or None)
        ...
        if args.delay_ms > 0 and index < len(selected_turns):
            time.sleep(args.delay_ms / 1000)

    result = {
        "mode": "rehearsal_counterparts_feishu_sequence_sent",
        "chat_id": chat_id,
