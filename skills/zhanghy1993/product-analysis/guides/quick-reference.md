...
| 操作 | 命令 |
|:---|:---|
| **创建输出目录** | `mkdir -p /home/claude/product-analysis-outputs/` |
| **验证 Mermaid 图** | `python3 scripts/validate_mermaid.py <文件路径>` |
| **最终质量检查** | `python3 scripts/run_quality_check.py <文件路径> [--type flow\|arch\|prd\|tracking]` |
| **MECE 原则检查** | `python3 scripts/check_mece.py <文件路径>` |
...
| 3. 埋点清单（快速版）| `assets/tracking-plan-lite.tpl.md` | 数据团队（快速交付） |
| 3. 埋点方案设计（标准版）| `assets/tracking-plan-template.md` | 数据团队（完整方案） |
