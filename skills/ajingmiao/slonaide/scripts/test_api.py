...
def test_api_connection(api_key):
    ...
    if records:
        ...
        if file_id:
            detail_url = f"{base_url}/api/audiofileMstr/audiofileToText"
            
            detail_payload = {
                'audiototextFileid': file_id,
                'audiototextLanguage': 'zh'
            }

            try:
                response = requests.post(detail_url, headers=headers, json=detail_payload, timeout=10)
                data = response.json()
