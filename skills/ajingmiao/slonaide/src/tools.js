...
export function registerSlonAideTools(api) {
  ...
  api.registerTool({
    ...
    async execute(_toolCallId, params) {
      try {
        ...
        // 获取笔记详情
        const response = await axios.post(
          `${baseUrl}/api/audiofileMstr/audiofileToText`,
          {
            audiototextFileid: params.fileId,
            audiototextLanguage: params.language || 'zh'
          },
          {
            headers: {
              'Authorization': `Bearer ${token}`,
  ...
  // 工具4:统一获取笔记（列表/摘要）
 api.registerTool({
 name: 'slonaide_get_notes',
 description: '统一获取 SlonAide录音笔记（view=list 或 view=summary）',
 parameters: {
 type: 'object',
 properties: {
 view: {
 type: 'string',
 description: '输出视图（list 或 summary）',
 default: 'list'
 },
 page: {
 type: 'number',
 description: '页码（仅 view=list 时生效，默认:1）',
 default:1
 },
 pageSize: {
 type: 'number',
 description: '每页数量（1-50；view=summary 时用于统计样本，默认:10）',
 default:10,
 minimum:1,
 maximum:50
 },
 count: {
 type: 'number',
 description: '摘要数量（1-10，仅 view=summary 时可用）',
 minimum:1,
 maximum:10
 },
 keyword: {
 type: 'string',
 description: '搜索关键词（仅 view=list 时可用）'
 }
 }
 },
 async execute(_toolCallId, params) {
 ...
 const config = initConfig();
 const baseUrl = config.baseUrl || 'https://api.aidenote.cn';
 const view = (params.view || 'list').toLowerCase();

 if (view !== 'list' && view !== 'summary') {
 throw new Error('view 参数仅支持 list 或 summary');
 }

 const resolvedPage = view === 'summary' ?1 : (params.page ||1);
 const resolvedPageSize = view === 'summary'
 ? (params.count || params.pageSize ||3)
 : (params.pageSize ||10);
 ...
 selectValue: view === 'list' ? (params.keyword || '') : ''
 },
 {
 headers: {
 'Authorization': `Bearer ${token}`,
 ...
 if (data.code !==200) {
 throw new Error(`获取笔记失败: ${data.message || '未知错误'}`);
 }
 ...
 return {
 content: [{ type: 'text', text: `❌ 获取笔记失败: ${error.message}` }]
 };
 }
 }
 });
