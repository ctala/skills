...
def main():
    parser = argparse.ArgumentParser(description="查询指定报告期全市场业绩预告（分页）")
    ...
    parser.add_argument("--report-type", required=True, choices=VALID_REPORT_TYPES,
                        help="报告期类型：q1（一季报）/ q2（中报）/ q3（三季报）/ annual（年报）")
    ...
    url = f"{BASE_URL}/data/api/v1/market/data/finance/stock-performance-forecast?" + urllib.parse.urlencode(params)
